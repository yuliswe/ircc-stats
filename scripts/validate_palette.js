...
export function validate(
  palette,
  { mode = 'light', surface, pairs = 'adjacent' } = {}
) {
  ...
  for (const kind of ['protan', 'deutan']) {
    for (const [i, j] of pairlist) {
      ...
      if (worst === null || d < worst[0])
        worst = [d, kind, palette[i], palette[j]];
    }
  }
  const tri = pairlist.length
    ? Math.min(
        ...pairlist.map(([i, j]) => deltaE(palette[i], palette[j], 'tritan'))
      )
    : 99;
  const nor = pairlist.length
    ? Math.min(...pairlist.map(([i, j]) => deltaE(palette[i], palette[j])))
    : 99;
  const wd = worst ? worst[0] : 99;
  const cvdState =
    wd >= CVD_TARGET ? 'pass' : wd >= CVD_FLOOR ? 'floor' : 'fail';
  if (cvdState === 'fail') ok = false;
  report.push([
    'CVD separation',
    cvdState,
    worst
      ? `worst ${label} ${worst[3]}↔${worst[2]} ΔE ${wd.toFixed(1)} (${worst[1]}) · tritan ${tri.toFixed(1)} · normal ${nor.toFixed(1)}`
      : 'n/a',
  ]);
...
export function validateOrdinal(palette, { mode = 'light', surface } = {}) {
  ...
  const thin = gaps
    .map((g, i) => [palette[i], palette[i + 1], +g.toFixed(3)])
  ...
  report.push([
    ...
    `${lightest} at ${cr.toFixed(2)}:1 vs surface` +
      (cr >= ORDINAL_LIGHT_FLOOR
        ? ''
        : ` — below ${ORDINAL_LIGHT_FLOOR}:1 floor`),
  ]);
...
function printReport({ report, ok }, { mode, surface, ordinal, n }) {
  ...
  for (const [name, state, detail] of report) {
    console.log(
      `  [${(GLYPH[state] ?? state).padEnd(4)}] ${name.padEnd(22)} ${detail}`
    );
  }
  if (ordinal) {
    console.log(
...
if (
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (VALUE_FLAGS.has(a)) {
      ...
      console.error(`unexpected extra positional: ${a}`);
      process.exit(2);
    }
  }
  for (const [k, allowed] of Object.entries(CHOICES)) {
    if (opts[k] != null && !allowed.includes(opts[k])) {
      console.error(
        `--${k} must be one of: ${allowed.join(', ')} (got ${JSON.stringify(opts[k])})`
      );
      process.exit(2);
    }
  }
  const palette = (positional || '')
    .split(',')
    .map(s => s.trim())
    .filter(Boolean);
  if (!palette.length) {
    console.error(
      'usage: node validate_palette.js "#hex,#hex,..." [--mode light|dark] [--surface #hex] [--pairs adjacent|all] [--ordinal]'
    );
    process.exit(2);
  }
  ...
  const result = opts.ordinal
    ...
    : validate(palette, { mode, surface, pairs });
  printReport(result, {
    mode,
    surface,
...
if (typeof document !== 'undefined') {
  ...
  if (b?.dataset.palette) {
    const palette = b.dataset.palette
      .split(',')
      .map(s => s.trim())
      .filter(Boolean);
    ...
    const result = ordinal
      ...
      : validate(palette, {
          mode,
          surface,
          pairs: b.dataset.pairs || 'adjacent',
        });
    console.table(
      result.report.map(([name, state, detail]) => ({
        check: name,
        result: GLYPH[state] ?? state,
        detail,
      }))
    );
    if (!result.ok)
      console.warn('validate_palette: FAILED — fix the marked checks');
  }
}
